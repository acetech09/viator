 ...
 * `x`-suffixed counts honoured, repeated lines preserved). Returns null when there is no
 * header line — i.e. the text isn't a fit.
 */
export function parseFit(text: string): ParsedFit | null {
  ...
  // isn't treated as an unmatched item).
  const body = raw.slice(headerIdx + 1).join('\n');
